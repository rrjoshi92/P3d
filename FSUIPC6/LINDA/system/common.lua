...
--### Initialise LINDA set up
    ...
    if sim >= 10 then
        P3D = 1
        _logg('[START] P3D or 64-bit found')
    else
        P3D = 0
    end
...
function switch (command, actions, group, knob_command)
	...
	_logggg('[COMM] SWITCH ' .. tostring(type(f)) .. ', C='
        .. tostring(command) .. ', A='
        .. tostring(actions) .. ', G='
        .. tostring(group) .. ', K='
        .. tostring(knob_command))
...
function Sounds(fname, norm, vol)
    ...
    i = 0
    p = 0
    -- extract Flt Sim path
    while (i < 256) and (p ~= nil) do
        p = string.find(FSpath, '\\', i)
        if p ~= nil then
            i = p + 1
        end
    end
    i = i - 1
    FSpath = string.sub (FSpath, 1, i)
    -- check and set sound path
    if norm == false or norm == nil then
        -- use LINDA sounds
        soundpath = FSpath .. 'modules/linda/data/sounds'
        if not file_exists(FSpath .. "modechange.wav") then
            soundpath = FSpath .. 'linda/data/sounds'
        end
    else
        -- use Flt Sim sounds
    ...
    _logggg('[COMM] Sound Path = ' .. soundpath .. fname ..
        ' vol=' .. soundvol)
