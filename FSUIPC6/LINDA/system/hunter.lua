...
function hunterAllLVars ()
	-- log
	_hnt ("All LVars watching request. Preparing...")
	-- generate list
	i = 0
	while i < 65536 do
		...
		LVARS[name] = val
		i = i + 1
		-- _hnt ("LVars new watch: " .. name .. " = " .. tostring(val))
	end
	_hnt ("All LVars watching started...")
end
...
function hunterListLVars ()
	-- log
	_hnt ("LVars list requested. Preparing...")
	local vars = {}
	-- generate list
	i = 0
	while i < 65536 do
...
function hunterSetLVar (name)
	...
	if LVARS[lvar] == nil then	_hnt ("LVar toggle: " .. lvar .. " = " .. tostring(val), 'L') end
end
